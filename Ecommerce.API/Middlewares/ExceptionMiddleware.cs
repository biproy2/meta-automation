...
public class ExceptionMiddleware(RequestDelegate next, ILogger<ExceptionMiddleware> logger)
{
    ...
    private static async Task HandleAsync(HttpContext ctx, Exception ex)
    {
        var (code, msg, errors) = ex switch
        {
            ...
            UnauthorizedException   => (HttpStatusCode.Unauthorized, ex.Message, Enumerable.Empty<string>()),
        ...
        {
            Success = false, StatusCode = (int)code, Message = msg,
            Errors = errors, TraceId = ctx.TraceIdentifier, Timestamp = DateTime.UtcNow
